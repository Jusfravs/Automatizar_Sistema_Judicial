...
if errorlevel 1 (
    echo ERROR: El comando codex no esta disponible en PATH.
    pause
    exit /b 1
)
...
for /f "delims=" %%F in ('where /r "%USERPROFILE%\.codex\packages\standalone\releases" codex-windows-sandbox-setup.exe 2^>nul') do (
    set "CODEX_RECURSOS_SANDBOX=%%~dpF"
    set "CODEX_EJECUTABLE_PROYECTO=%%~dpF..\bin\codex.exe"
)
